...
impl StereoChorus {
    ...
    pub fn process(&mut self, input: f32) -> (f32, f32) {
        ...
        // Triangle wave: two phases offset by 0.25 (90°) for stereo
        let tri_left = triangle(self.lfo_phase);
        let tri_right = triangle((self.lfo_phase + 0.25) % 1.0);

        // Center delay ~7ms, depth sweeps ±0-8ms around it
        let center_samples = 0.007 * self.sample_rate; // 7ms
        let depth_samples = self.depth * 0.008 * self.sample_rate; // 0-8ms
        ...
        // BBD lowpass filter (anti-aliasing character = warmth)
        let wet_left = self.lp_left.process(wet_left);
        let wet_right = self.lp_right.process(wet_right);

        // Mix: at mix=0 → pure dry (mono), at mix=1 → pure wet (stereo)
        let dry = input;
...
mod tests {
    ...
    fn test_chorus_creates_stereo_difference() {
        ...
        // Left and right should differ (phase-offset LFO creates different delay modulation)
