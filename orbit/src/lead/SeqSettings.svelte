<script lang="ts">
    ...
    import {
        seqDirection, seqSwing, seqTimeDivision,
        setSeqDirection, setSeqSwing, setSeqTimeDivision,
        rotatePattern, randomizeGates, clearSequence,
    } from './stores/state';

    $: dir = $seqDirection;
    $: swing = $seqSwing;
    $: tdiv = $seqTimeDivision;
...
{#each directions as d}
    <button class="btn" class:active={dir === d.value} onclick={() => setSeqDirection(d.value)} style="--c: {LEAD_COLOUR}">{d.label}</button>
{/each}
<span class="sep"></span>
<span class="lbl">RATE</span>
{#each timeDivs as t}
    <button class="btn" class:active={tdiv === t.value} onclick={() => setSeqTimeDivision(t.value)} style="--c: {LEAD_COLOUR}">{t.label}</button>
{/each}
<span class="sep"></span>
<span class="lbl">SWG</span>
<input type="range" min="0" max="100" step="1" value={swing}
    oninput={(e) => setSeqSwing(Number((e.target as HTMLInputElement).value))}
    class="inline-slider" style="--c: {LEAD_COLOUR}; --fill-pct: {swing}%"
/>
<span class="val" style="color: {LEAD_COLOUR}">{swing}</span>
<span class="sep"></span>
<button class="btn" onclick={() => rotatePattern(-1)} style="--c: {LEAD_COLOUR}">←</button>
<button class="btn" onclick={() => rotatePattern(1)} style="--c: {LEAD_COLOUR}">→</button>
<button class="btn" onclick={randomizeGates} style="--c: {LEAD_COLOUR}">RND</button>
<button class="btn clr" onclick={clearSequence}>CLR</button>
