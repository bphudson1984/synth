<script lang="ts">
    ...
    import {
        seqSteps, seqSelectedStep,
        setStepVelocity, setStepGatePct,
        setStepProbability, setStepRatchet, toggleStepSkip,
    } from './stores/state';
...
{#if hasStep}
    ...
    <input type="range" min="0" max="100" step="1" value={step.velocity * 100 / 127}
        oninput={(e) => setStepVelocity(Number((e.target as HTMLInputElement).value))}
        class="inline-slider" style="--c: {LEAD_COLOUR}; --fill-pct: {step.velocity * 100 / 127}%"
    />
    <span class="val" style="color: {LEAD_COLOUR}">{step.velocity}</span>
    <span class="sep"></span>
    <span class="lbl">GATE</span>
    <input type="range" min="5" max="100" step="1" value={step.gatePct}
        oninput={(e) => setStepGatePct(Number((e.target as HTMLInputElement).value))}
        class="inline-slider" style="--c: {LEAD_COLOUR}; --fill-pct: {step.gatePct}%"
    />
    <span class="val" style="color: {LEAD_COLOUR}">{step.gatePct}%</span>
    <span class="sep"></span>
    <span class="lbl">PROB</span>
    <input type="range" min="0" max="100" step="1" value={step.probability}
        oninput={(e) => setStepProbability(Number((e.target as HTMLInputElement).value))}
        class="inline-slider" style="--c: {LEAD_COLOUR}; --fill-pct: {step.probability}%"
    />
    <span class="val" style="color: {LEAD_COLOUR}">{step.probability}%</span>
    <span class="sep"></span>
    <span class="lbl">RTCH</span>
    {#each [1, 2, 3, 4] as r}
        <button class="btn" class:active={step.ratchet === r} onclick={() => setStepRatchet(r)} style="--c: {LEAD_COLOUR}">{r}x</button>
    {/each}
    <span class="sep"></span>
    <button class="btn skip" class:active={step.skip} onclick={toggleStepSkip}>SKIP</button>
{:else}
...
<style>
    ...
    .inline-slider {
        ...
        background: linear-gradient(to right, var(--c) 0%, var(--c) var(--fill-pct), var(--orbit-border) var(--fill-pct));
